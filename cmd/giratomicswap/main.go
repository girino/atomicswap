...
var (
	...
	connectFlag = flagset.String("s", "localhost", "host[:port] of GirinoCoin Core wallet RPC server")
...
// GirinoCoin transactions for these swaps.  A second tool should be used for the
// transaction on the other chain.  Any chain can be used so long as it supports
// OP_SHA256 and OP_CHECKLOCKTIMEVERIFY.
//
// Example scenerios using girinocoin as the second chain:
//
// Scenerio 1:
//   cp1 initiates (dcr)
//   cp2 participates with cp1 H(S) (gir)
//   cp1 redeems gir revealing S
...
//     - must verify H(S) in contract is hash of known secret
//   cp2 redeems gir with S

func init() {
	flagset.Usage = func() {
		fmt.Println("Usage: giratomicswap [flags] cmd [cmd args]")
		fmt.Println()
...
// pubkey for a transaction input signature.  Due to limitations of the GirinoCoin
...
// account parameter which was removed in GirinoCoin Core 0.15.
...
func (cmd *initiateCmd) runCommand(c *rpc.Client) error {
	...
	fmt.Printf("Contract fee: %v (%0.8f GIR/kB)\n", b.contractFee, contractFeePerKb)
	fmt.Printf("Refund fee:   %v (%0.8f GIR/kB)\n\n", b.refundFee, refundFeePerKb)
...
func (cmd *redeemCmd) runCommand(c *rpc.Client) error {
	...
	fmt.Printf("Redeem fee: %v (%0.8f GIR/kB)\n\n", fee, redeemFeePerKb)
